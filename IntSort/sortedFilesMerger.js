...
class SortedFilesMerger extends EventEmitter {
	...
	mergeSortedFiles() {
		return new Promise((resolve, reject) => {
			...
			//pause itself
			const pauseStreams = inputFileStreams.map(() => new Bacon.Bus());
	 ...
	 */
	_closeFileStreams() {
		const closeFileStream = fileStream => fileStream.close();
		
		//Close all the input file streams
		this.inputFileStreams.forEach(closeFileStream);
		
		//Close the output stream
		closeFileStream(this.outputFileStream);		
	}
	 ...
	 */
	_setDataStreamPaused(pauseStream, pause) {
	 ...
	 * 	current value.
	 */
	_findMinValueInfo(minValueInfo, currentValue, currentIndex) {
